<template>
    <section class="segundaSection">
        <div class="container">
            <div class="row">
                <div class="col-xl-12 np titleEstablecimientos">
                    <h1>
                        Encuentra el mejor negocio en tu ciudad
                    </h1>
                </div>
                <div class="col-6 col-md-4 col-lg-3 col-xl-3 text-center padCategoria">
                    <a href="">
                        <div class="col-12 np">
    
                            <img class="" src="../../static/images/hotel.png" alt="">
    
                        </div>
                        <div class="col-12 np postitleCategoria">
                            <h6>
                                Hoteles
                            </h6>
                        </div>
                    </a>
                </div>
                <div class="col-6 col-md-4 col-lg-3 col-xl-3 text-center padCategoria">
                    <a href="">
                        <div class="col-12 np">
    
                            <img src="../../static/images/food-truck.png" alt="">
    
                        </div>
                        <div class="col-12 np postitleCategoria">
                            <h6>
                                Comida Rápida
                            </h6>
                        </div>
                    </a>
                </div>
                <div class="col-6 col-md-4 col-lg-3 col-xl-3 text-center padCategoria">
                    <a href="">
                        <div class="col-12 np">
    
                            <img src="../../static/images/restaurant.png" alt="">
    
                        </div>
                        <div class="col-12 np postitleCategoria">
                            <h6>
                                Restaurantes
                            </h6>
                        </div>
                    </a>
                </div>
                <div class="col-6 col-md-4 col-lg-3 col-xl-3 text-center padCategoria">
                    <a href="">
                        <div class="col-12 np">
    
                            <img src="../../static/images/gym.png" alt="">
    
                        </div>
                        <div class="col-12 np postitleCategoria">
                            <h6>
                                Gimnasios
                            </h6>
                        </div>
                    </a>
                </div>
            </div>
        </div>
    </section>
</template>

<script>
export default {
    name: 'CategoriasHome',
}
</script>
